...
if not env_loaded:
    print("⚠️  No .env file found. Searched in:")
...
class Settings:
    ...
    ENCRYPTION_KEY: str = os.getenv(
        "ENCRYPTION_KEY", ""
    )  # For database credential encryption
    ...
    DB_CONNECTION_TIMEOUT_MS: int = int(
        os.getenv("DB_CONNECTION_TIMEOUT_MS", "20000")
    )  # 20 seconds
    DB_SERVER_SELECTION_TIMEOUT_MS: int = int(
        os.getenv("DB_SERVER_SELECTION_TIMEOUT_MS", "10000")
    )  # 10 seconds
    ...
    def validate_aws_credentials(self) -> dict:
        ...
        return {
            ...
            "aws_access_key_length": len(self.AWS_ACCESS_KEY_ID)
            if self.AWS_ACCESS_KEY_ID
            else 0,
            "aws_secret_key_present": bool(self.AWS_SECRET_ACCESS_KEY),
            "aws_secret_key_length": len(self.AWS_SECRET_ACCESS_KEY)
