...
from db.session import DatabaseManager, get_database_manager
...
async def process_custom_query(
    ...
    try:
        result = await service.process_custom_query(
            connection_id=request.connection_id, user_query=request.user_query
        )
        ...
        raise HTTPException(
            status_code=500, detail=f"Error processing custom query: {str(e)}"
        )
